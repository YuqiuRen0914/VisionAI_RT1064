...
| `0x07` | SENSOR_INVALID 编码器/IMU 数据无效或长时间不更新  |
| `0x08` | CONTROL_UNSTABLE 闭环振荡、超调或无法进入完成窗口 |
...
1. **底盘类型**：差速 / 阿克曼 / 麦克纳姆？— 影响 LEFT/RIGHT 是否需要上位机拆动作。
2. **车头方向定义**：开机时车头朝哪定义为 UP？是否需要上位机发"初始化朝向"命令？
3. **DONE 是否带误差反馈**：是否需要把"实际走了多少 cm"也回传？(本版只回耗时；如要扩展，可加 v1.1)
4. **是否需要主动上报**：例如电机过流、电池低压等异步事件，是否走 STATUS 帧主动 push？
5. **网格步长**：上位机将以 cm 发 MOVE，下位机内部如何把 cm 映射到编码器圈数？(标定方法约定)
