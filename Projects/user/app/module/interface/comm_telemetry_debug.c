                                                ...
                                                comm_telemetry_debug_write_line_t write_line)
{
    ...
    {
        comm_debug_send_line(write_line, "ERR imu no_data");
        return;
    }
...
static void comm_telemetry_debug_update_imu_stat(comm_telemetry_debug_write_line_t write_line)
{
                                            ...
                                            &yaw_x10) != AI_OK)
    {
        ...
        comm_debug_send_line(write_line, "ERR imu stat no_data");
        return;
    }
    ...
    {
        comm_telemetry_imu_stat.active = 0U;
        comm_debug_send_line(write_line, "ERR imu stat no_samples");
        return;
    }
...
void comm_telemetry_debug_handle_stream(char *mode, comm_telemetry_debug_write_line_t write_line)
{
    ...
    if(mode == 0)
    {
        comm_debug_send_line(write_line, "ERR stream missing_mode");
        return;
    }
    ...
    {
        comm_debug_send_line(write_line, "ERR stream bad_mode=%s", mode);
        return;
    }
...
void comm_telemetry_debug_handle_imu(char *sub_command, comm_telemetry_debug_write_line_t write_line)
{
    ...
    if(sub_command == 0)
    {
        comm_debug_send_line(write_line, "ERR imu missing_command");
        return;
    }
    ...
    if(strcmp(sub_command, "zero") == 0)
    {
        Imu660raResetYaw();
        calibration_yawx_zero(&comm_telemetry_cal_yawx, 0);
        comm_debug_send_line(write_line, "OK imu zero");
    }
    else if(strcmp(sub_command, "yawx") == 0)
    {
        ...
        if((yawx_command != 0) && (strcmp(yawx_command, "zero") == 0))
        {
            Imu660raResetYaw();
            calibration_yawx_zero(&comm_telemetry_cal_yawx, 0);
            comm_debug_send_line(write_line, "OK imu yawx zero");
        }
        else
        {
            comm_debug_send_line(write_line, "ERR imu yawx usage");
        }
    }
    else if(strcmp(sub_command, "stat") == 0)
    {
        ...
        if(comm_debug_parse_i32(duration_text, &duration_ms) != AI_OK)
        {
            comm_debug_send_line(write_line, "ERR imu stat bad_ms");
            return;
        }
    ...
    {
        comm_debug_send_line(write_line, "ERR imu bad_command=%s", sub_command);
    }
}
