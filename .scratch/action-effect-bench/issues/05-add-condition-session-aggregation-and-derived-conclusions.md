...
- [x] JSON output is layered into `session`, `conditions`, and `runs`
- [x] Markdown output contains both per-condition aggregate sections and per-run detail rows
- [x] Per-condition aggregates use pass counts, quality counts, and median/min/max rather than mean-first reporting
- [x] Session-level `primary_status` is derived according to the agreed rules for `accepted`, `scoped-pass`, `rejected`, `exploratory`, `partial`, and `aborted`
- [x] `quality_flag` is produced as a separate field and triggered by the agreed concern rules
- [x] Repeated sessions for the same candidate label are not auto-collapsed into a single cross-session verdict
